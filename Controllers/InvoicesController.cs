...
public class InvoicesController : ControllerBase
{
    ...
    public async Task<IActionResult> ConfirmPayment([FromBody] ConfirmPaymentDto dto)
    {
        ...
        try
        {
            ...
            // - Nếu ca khám có đơn thuốc nhưng Dược sĩ CHƯA phát thuốc xong -> chuyển sang StatusId = 10 (Chờ phát thuốc)
            ...
            if (appt != null)
            {
                ...
                var rxList = medRecord != null ? await _context.Prescriptions.Where(p => p.MedicalRecordId == medRecord.MedicalRecordId).ToListAsync() : new List<Prescription>();
                bool hasPendingRx = rxList.Any(p => p.Status != "Completed" && p.Status != "Dispensed");
